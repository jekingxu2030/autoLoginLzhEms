...
"""email_sender.py – WIC‑Power 内部邮件发送模块
================================================
已预设 WIC‑Power 邮件服务器参数，开箱即用：

```python
from email_sender import send_email
send_email("bob@wic-power.cn", "测试", "你好 Bob！")
```

默认配置
--------
* 协议：SMTP（POP3 仅用于收件，此模块负责发信）
* 服务器：``email.wic-power.cn``
* 端口：``25``（不启用 SSL/TLS）
* 账户：``sysinfo.wic-power.cn``  （请确认是否需要 ``@wic-power.cn``）
* 密码：``wicpower``

如果在生产环境需要修改，可通过下列环境变量覆盖：

* ``SMTP_HOST`` | ``SMTP_PORT`` | ``SMTP_USERNAME`` | ``SMTP_PASSWORD``
* ``SMTP_USE_TLS``（``true``/``false``，默认 *false*）
* ``SMTP_DEBUG``（``true``/``false``）

依赖全部来自 Python 标准库，无需额外安装。
"""
...
#     # ------------------------------------------------------------------
#     def _make_msg(self, to_addr: str, subject: str, body: str) -> MIMEMultipart:
#         msg = MIMEMultipart()
...
#         """发送邮件并返回结果。"""
#         print(f"⚡ Connecting → {self.smtp_host}:{self.smtp_port} (TLS={self.use_tls})")
#         try:
...
#                 print("⚡ LOGIN …")
#                 code, reply = server.login(self.username, self.password)
#                 print(f"LOGIN → {code} {reply.decode(errors='ignore')}")

#                 msg = self._make_msg(to_addr, subject, body)
#                 print(f"⚡ SENDMAIL → {to_addr}")
#                 response = server.sendmail(self.username, [to_addr], msg.as_string())

#                 if response == {}:
#                     print("✔ 发送成功！")
#                     return {"success": True, "response": "250 OK"}

#                 print(f"✖ 部分失败: {response}")
...
#     """快捷函数：调用默认配置直接发信。"""
#     return _default_sender.send(to_addr, subject, body)
...
# USE_TLS = true # 默认
DEBUG = os.getenv("SMTP_DEBUG", "false").lower() == "true"
DEFAULT_DOMAIN = os.getenv("SMTP_DEFAULT_DOMAIN", "smtp.qq.com")
