			...
			async with websockets.connect(ws_url, extra_headers=headers) as ws:
				...
				for _ in range(3):	# 最多等待 5 条数据
					...
					if data.get("func") == "menu":
						print(f"[MENU] 收到 menu 数据，字段数量: {len(data.get('data', {}))}")
						# print(f"[MENU]数据: {data[1]}")   #---字典不能切分打印
						# print(f"[MENU]数据（前10项）: {dict(list(data.items())[:10])}")
						print(f"[MENU]Data数据的第一层键名: {list(data.get('data', {}).keys())}")
...
class EmsWsMonitor:
	def __init__(self, driver, timeout=35, menu_data=None):
		self.driver = driver
		...
		try:
			if self.menu_data:
				...
				for device_type, field_list in menu_cache.items():
					print(f"[MENU] 设备类型: {device_type}，字段数量: {len(field_list)}")
					...
					try:
						...
						if message["method"] == "Network.webSocketFrameReceived":
							...
							if data.get("func") == "rtv":
								...
								print(f"\n[WS拦截] 收到RTV数据帧: {payload[:100]}...")

								rtv_data = data.get("data", [])
								print(f"\n[WS拦截] 共接收 {len(rtv_data)} 个字段ID")
								# 增加详细日志
								# print(f"[WS调试] 接收数据帧长度: {len(rtv_data)}")
								print(f"[WS调试] 第一个数据样本: {rtv_data[0] if rtv_data else '无数据'}")
								for item in rtv_data:
									val = item.get("value")
									if val not in ("", None, 87, "87") and str(val).strip().upper() not in (
										"NULL",
										"NAN",
									):
								...
								if hasattr(self, "menu_cache"):	# 确保 menu_cache 存在
									...
									os.makedirs(folder_path, exist_ok=True)	# 若目录不存在则创建
								...
								print(f"\n[WS拦截] 未知定位类型数据: {payload[:50]}...")

					except Exception as e:
						print(f"[WS监听异常] {e}")
						return "❌error"	# 未知错误
				time.sleep(1)
			if not self.msg_arrived:	# 无数据
				return "❌no_msg"
			elif not self.data_valid:	# 有87或空字符数据
				return "❌empty"
			else:
				return "✅ok"
		except Exception as e:
