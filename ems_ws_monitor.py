        ...
        try:
                    ...
                    try:
                        ...
                        if message["method"] == "Network.webSocketFrameReceived":
                            ...
                            if data.get("func") == "rtv":
                                ...
                                print(
                                    f"[WS调试] 第一个数据样本: {rtv_data[0] if rtv_data else '无数据'}"
                                )
                                for item in rtv_data:
                                    val = item.get("value")
                                    if val not in ("", None, 87, "87") and str(
                                        val
                                    ).strip().upper() not in (
                                        "NULL",
                                        "NAN",
                                ...
                                if hasattr(self, "menu_cache"):  # 确保 menu_cache 存在
                                    ...
                                    folder_path = os.path.join(
                                        os.path.dirname(__file__), "dataLog"
                                    )
                                    os.makedirs(
            ...
            if not self.msg_arrived:  # 无数据
                ...
                return "❌empty"
                 ...
                 print(f"[WS状态] 数据接收正常，有效字段确认，返回✅ok")
                 return "✅ok"
            else:
                print(f"[WS状态]返回超时，返回❌timeout")
                return "❌timeout"
        except Exception as e:
            print(f"[WS错误] {e}")
            return "❌no_ws"  # 连接错误
